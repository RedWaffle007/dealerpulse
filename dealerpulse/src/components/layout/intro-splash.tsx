...
function ringTick() {
  try {
    ...
    audio.volume = 0.7;
    // Autoplay may be blocked without a prior gesture; swallow that rejection so
    // it never surfaces as an unhandled error, and retry on the first
    // interaction so the strike still rings once the user clicks/taps/keys.
    audio.play().catch(() => {
      const playOnce = () => {
        audio.play().catch(() => {});
        cleanup();
